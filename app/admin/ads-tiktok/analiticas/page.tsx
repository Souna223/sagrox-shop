...
export const metadata: Metadata = {
  title: "Analíticas TikTok",
};

export default async function AdminAdsTikTokAnaliticasPage({
  searchParams,
}: {
  ...
  const validRange = (["7", "30", "90"] as AdsAnalyticsRange[]).includes(range as AdsAnalyticsRange)
    ...
    : "30";
  const data = await getAdsAnalytics("tiktok", Number(validRange));

  return <AdsAnalyticsView platform="tiktok" range={validRange} data={data} />;
}